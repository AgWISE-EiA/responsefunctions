...
# Summarise to view the number of trials per treatment in each experiment
df2 <- ds %>% 
  dplyr::group_by(TLID,Treatment) %>% 
...
# Summarise and explore data to view the number of trials per treatment in each experiment
df3 <- ds %>% 
...
gg6 <- ggplot(ds2, aes(x = refY, y = yieldEffectBlup)) + 
  ...
  theme(axis.title = element_text(size = 14, face="bold"),
        ...
        legend.position = "none")
gg6
ggsave(paste(pathIn, "BLUP_reducingRandomError70N.pdf", sep=""), grid.arrange(gg5, gg6, ncol=2), width=12, height=8)
...
rec_targetdY <- function(my_ferts,
  ...
  if(GYt > att_GY){
    ...
    #build the function to optimize
    ...
    #now performing the optimization that minimizes the difference to the target yield:
    result <- optim(start,
                    qfo,
                    my_ferts = my_ferts,
                    GYt = GYt,
